...
/** A pollar count at the remote guideway.
  */
final class GuidewayCount
{
    /* * *
    - redundant requests
    ...
    /** Constructs a GuidewayCount.
      */
    @ThreadSafe GuidewayCount( final String pollName ) { this.pollName = pollName; }
   ...
   // --------------------------------------------------------------------------------------------------
    ...
    /** Asynchronously requests the initial data for a group of peers.  The result set will include all
      ...
      *     @param receiver The agent to handle any eventual response from the remote guideway;
    ...
    void enqueuePeersRequest( final VotingID rootwardID, final PeersReceiver receiver, final int paddedLimit )
    {
        ...
        receiver.receivePeersResponse( rootwardID ); // encoding default response, pending real guideway counts
    }


        /* * *
        - guideway may implement by querying for all peers of ordinal <= paddedLimit
    ...
    /** Asynchronously requests the data for a range of minor peers.  The order of peers in the result
      ...
      *     @param receiver The agent to handle any eventual response from the remote guideway; normally
    ...
    void enqueuePeersRequest( VotingID rootwardID, PeersReceiver receiver, int peersStart, int peersEndBound )
    {
        ...
        throw new UnsupportedOperationException( "Guideway counts not yet coded" );
    }
    ...
    /** Synchronously connects to the remote guideway and returns from the latest count the identified
    ...
    UnadjustedNode fetchNode( final VotingID id, final Map<VotingID,UnadjustedNode> nodeMap )
    {
        ...
        return null; // guideway counts not yet coded
    }
