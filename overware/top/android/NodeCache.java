...
/** A partial store of the nodes that contribute to a {@linkplain Forest pollar forest}.  Although it
  ...
  * {@linkplain GuidewayCount#enqueuePeersRequest(VotingID,PeersReceiver,int) on demand} as further
  * nodal data is received from the guideway count engine.
  */
interface NodeCache
{
    /* * *
    - cache inconsistency due to lazy growth is detected by repocaster serial number (RepocastSer)
        ...
        - to detect
            - remote guideway (re)stamps with current repocaster serial number
